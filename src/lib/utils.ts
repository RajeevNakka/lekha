...
export function toDatetimeLocalFormat(date: string | Date): string {
    if (!date) {
        // Return current date/time if no date provided
    ...
    if (isNaN(d.getTime())) {
        console.error('Invalid date provided to toDatetimeLocalFormat:', date);
        return toDatetimeLocalFormat(new Date());
    }

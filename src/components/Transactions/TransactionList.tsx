                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            <div className="md:hidden">
                                {filteredTransactions.map((tx) => (
                                    <div key={tx.id} className="p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex-1 mr-4">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="text-xs text-gray-500">{formatDateTime(tx.date)}</span>
                                            ...
                                            <span className={`font-bold whitespace-nowrap
                                                ${tx.type === 'income' ? 'text-green-600' :
                                                    tx.type === 'expense' ? 'text-red-600' :
                                                        'text-blue-600'}`}>
                                                {tx.type === 'expense' ? '-' : '+'}{formatCurrency(Number(getFieldValue(tx, 'amount')), activeBook.currency)}
                                            </span>
                                        </div>

                                        <div className="flex justify-between items-center mt-3">
                                            <div className="text-xs text-gray-500 truncate max-w-[150px]">
                                                {getFieldValue(tx, 'category_id') || getFieldValue(tx, 'category') || '-'}
                                            </div>
                                            <div className="flex gap-3">
                                                <button
